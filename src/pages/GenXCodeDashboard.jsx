...
// 1. IMPORT FIX: Make sure to import your new GenXCode domains here
import { genxcodeDomains } from "@/data/genxcodeDomains"

export default function GenXCodeDashboard() {
  const navigate = useNavigate()
  
  // 2. STATE FIX: Change the default domainKey to "arcade_event"
  ...
  useEffect(() => {
    const generateInitialId = async () => {
      try {
        const newId = await getNextCertificateId(formData.domainKey, 'GENX') 
  ...
  // 3. VARIABLE FIX: Use genxcodeDomains instead of the old domains object
  ...
  const handleExportPdf = async () => {
    try {
      ...
      if (!formData.studentEmail) throw new Error("Student email is required.");
      ...
      if (!emailResponse.ok || !result.success) {
        throw new Error(result.message || "Saved, but failed to send email.");
      }

      alert("GenXCode Certificate Exported & Email Sent Successfully!");
    } catch (error) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-purple-50">
      <div className="max-w-[1600px] mx-auto grid grid-cols-[420px_1fr] gap-8 p-8">
        ...
        <div className="bg-white/80 backdrop-blur-xl rounded-[28px] shadow-xl border border-purple-100 p-8 h-fit sticky top-8">
          <div className="mb-8">
            <h2 className="text-2xl font-bold text-[#1b053e]" style={{ fontFamily: '"Times New Roman", serif' }}>GenXCode Certificate</h2>
          ...
          <div className="space-y-6">
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">Event Track</label>
              <select
              ...
              >
                {/* MATCHING THE DROPDOWN WITH genxcodeDomains.js */}
                <option value="arcade_event">GenXCode Arcade</option>
                <option value="coding_challenge">Rapid Coding Challenge</option>
                <option value="ui_hackathon">UI/UX Design Sprint</option>
              </select>
            </div>
            ...
            {/* Certificate ID Box styled for GenX */}
            <div className="bg-gradient-to-br from-[#1b053e] to-[#3d126b] rounded-3xl p-6 shadow-lg">
              <div className="flex items-center justify-between">
                <p className="text-xs uppercase tracking-[0.25em] text-[#c7a6f3]">Certificate No.</p>
                <button
                  type="button"
        ...
        <div ref={previewRef} className="pb-20" style={{ width: "100%" }}>
          <div style={{ width: CERT_WIDTH * scale, height: CERT_HEIGHT * scale }}>
            ...
            >
              <GenXCodeCertificate
                studentName={formData.studentName}
                domainName={selectedDomain?.domainName ?? ""} // <--- NOW PASSING CORRECT DATA
                role={selectedDomain?.role ?? ""}             // <--- NOW PASSING CORRECT DATA
